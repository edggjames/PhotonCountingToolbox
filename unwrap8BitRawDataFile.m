...
% Function to extract photon arrival time series from 8 bit Flex02 raw data 
...
% should be 8 bit here
if file_format ~= 8
    disp('Error - date file is not in 8 bit format')
    % return a vector of NaNs
    [f,T,duration,num_photons,abs_photon_times_clock,abs_photon_times_seconds, ...
    rel_photon_times_clock, rel_photon_times_seconds] = NaN(8,1);
    return
end
...
rel_photon_times_clock = zeros(num_photons+1,1);
